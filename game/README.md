...
- [x] 布局与氛围升级：2 格窄路 / 图书馆石板广场 /
      作坊街地块 / 出生地花圃+路牌+长椅；新增装饰素材（灯柱/知识方尖碑/
      学院旗帜/展开书台/长椅/路牌/蝴蝶）；持续动画——池塘水面波动、开工作坊烟囱炊烟、
...
- [x] 辅助管理员操控（2026-06-13）：把世界里已有的**辅助管理员**
      当作玩家化身——左下角"🎮 操控辅助管理员"开关（或按 G）进入跟随视角，相机锁定该角色，
      **WSAD** 移动、走到其它 AI 成员附近（≤96px）头顶提示"按 F 交互"、按 **F** 在底部
      面板打开该成员信息；受控期间隐藏血条、暂停自治游荡（释放后回锚区）。
      不再额外生成化身，避免重复显示；无 token 上限的角色统一不画血条
      （`MemberActor.setControlled` / `refreshTokenBar`）
- [x] 布局与建筑体量调整（2026-06-13）：
...
- [x] 环境整体升级（2026-07-02）：**树木 4 变体**（橡树/松树/白桦/樱花，24x36@1x 大树 +
      随机缩放/镜像），边缘林带环抱小镇（带缺口）+ 内部树丛聚簇 + 稀有樱花地标；
      **灌木丛**贴图（圆/浆果/花 3 变体，树脚伴生 + 草地小簇）；地表瓦片集扩至 24 格
      （新增蓝白野花/蘑菇/裸土/嫩芽/落叶），草色与花甸按低频值噪声成片分布；
      **动态效果**——树冠微风摇曳（update 统一 sin 驱动）、樱花花瓣飘落（对象池）；
      蝴蝶改为"家园锚点"游荡（`BUTTERFLY_HOMES`），分散全图花草区，不再全图直线穿越挤在中部
- [x] 小游戏模式（2026-07-13）：池塘右侧新增"游乐角"三座小游戏建筑——
      贪吃蛇街机屋 / 2048 数字馆 / 方块塔（新增 `building_arcade_*.png` 三套 4 帧贴图 +
      脚下石板地坪 + 头顶名牌），点击建筑弹出对应小游戏（`ui/minigames/`：
      贪吃蛇 / 2048 / 俄罗斯方块，纯 canvas 实现，Esc 关闭、↻ 重开、
      最高分 localStorage 持久化，弹窗期间停用场景键盘避免按键穿透世界）
...
game/
  ...
  src/
    ...
    assets.ts       ← 运行时资源 URL：PNG / SFX / BGM 的统一入口
    ...
    ui/minigames/   ← 小游戏（池塘右侧游乐角建筑，点击弹窗游玩）
      index.ts      ← 游戏注册表 + 弹窗外壳（得分/最高分/重开/关闭）
      snake.ts game2048.ts tetris.ts ← 三个游戏本体（纯 canvas，零依赖）

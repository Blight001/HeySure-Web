...
- **重依赖组件必须懒加载**：`@xterm`（`RemoteTerminalPane` 由远控工作台动态加载）、大体量面板（KnowledgeBasePanel/WorkshopPanel）均用 `defineAsyncComponent` 按需拆包，新增大依赖时同样处理。
- **远控 Connector 地址**：登录响应的 `agent_socket_url` 优先，其次是 `VITE_CONNECTOR_SOCKET_URL`；未配置时 HTTP 直连同主机 `:3002`，HTTPS 才回退同源 `/socket.io`。画面和终端禁止各自硬编码地址。
